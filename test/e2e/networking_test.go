...
var _ = Describe("networking", Label("required", "networking"), func() {
	...
	It("should create and validate HostNetworkAttachment resources", func() {
		...
		Expect(retrieved.Spec.AllowedVLANs).To(Equal([]int{100, 200, 300}))
	...
	It("should re-apply port configs when NetworkInterfaces change on available host", func() {
		...
		By("waiting for the BMH to enter preparing")
		WaitForBmhInProvisioningState(ctx, WaitForBmhInProvisioningStateInput{
			Client: c,
			Bmh:    *bmh,
		...
		By("verifying AppliedPortConfigs updated to new VLAN")
		Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
		Expect(bmh.Status.AppliedPortConfigs).To(HaveLen(1))
		Expect(bmh.Status.AppliedPortConfigs[0].SwitchPortConfig.NativeVLAN).To(Equal(200))
		...
		By("verifying AppliedPortConfigs is cleared and condition is removed")
		Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
		Expect(bmh.Status.AppliedPortConfigs).To(BeEmpty())
		cond := meta.FindStatusCondition(bmh.Status.Conditions, metal3api.NetworkInterfacesValidCondition)
		Expect(cond).To(BeNil(), "NetworkInterfacesValid condition should be removed when no interfaces")
	...
	It("should maintain port configs during deprovisioning", func() {
		...
		By("verifying port config is cleared after NI removal")
		Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
		Expect(bmh.Status.AppliedPortConfigs).To(BeEmpty())
