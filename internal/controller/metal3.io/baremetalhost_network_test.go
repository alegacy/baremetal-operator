...
func TestValidateNetworkInterfaces(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedDirty:          true,
			expectedValidationPass: false,
			expectedReason:         "HardwareDiscoveryIncomplete",
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			dirty, err := r.validateNetworkInterfaces(context.Background(), tc.host)
...
func TestSwitchPortConfigurationNeedsUpdate(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			portConfigs: map[string]*provisioner.PortConfig{
				"00:11:22:33:44:55": {SwitchPortConfig: provisioner.SwitchPortConfig{Mode: "access", NativeVLAN: 100}},
			},
			expectedUpdate: true,
		},
		{
...
func TestResolveSwitchPortConfigs(t *testing.T) {
	...
	configs, err := r.resolvePortConfigs(context.TODO(), host)
	require.NoError(t, err)
	assert.Len(t, configs, 1)
	assert.Contains(t, configs, "aa:bb:cc:dd:ee:f0")
	assert.Equal(t, "access", configs["aa:bb:cc:dd:ee:f0"].SwitchPortConfig.Mode)
...
func TestResolveSwitchPortConfigsCrossNamespace(t *testing.T) {
	...
	assert.Len(t, configs, 1)
	assert.Contains(t, configs, "aa:bb:cc:dd:ee:f0")
	assert.Equal(t, "trunk", configs["aa:bb:cc:dd:ee:f0"].SwitchPortConfig.Mode)
	...
	host := &metal3api.BareMetalHost{
		...
		Spec: metal3api.BareMetalHostSpec{
			NetworkInterfaces: []metal3api.NetworkInterface{
				{
					MACAddress: "00:11:22:33:44:55",
					SwitchPort: &metal3api.SwitchPort{
...
func TestResolveSwitchPortConfigsWithHNAAndSwitchPort(t *testing.T) {
	...
	assert.Equal(t, "access", cfg.SwitchPortConfig.Mode)
...
func TestValidateNetworkInterfacesMissingAttachment(t *testing.T) {
	...
	dirty, err := r.validateNetworkInterfaces(context.Background(), host)
	require.NoError(t, err)
	assert.True(t, dirty)
				...
				AppliedPortConfigs: []metal3api.AppliedPortConfig{
					{
						...
						LocalLinkConnection: &metal3api.LocalLinkConnection{SwitchID: "aa:bb:cc:dd:ee:ff", PortID: "Ethernet1/1"},
					},
				},
			},
		}
		...
		info := &reconcileInfo{
			...
			portConfigs: map[string]*provisioner.PortConfig{
				"00:11:22:33:44:55": {
					...
					LocalLinkConnection: &provisioner.LocalLinkConnection{SwitchID: "aa:bb:cc:dd:ee:ff", PortID: "Ethernet1/1"},
				},
			},
		}
	...
	t.Run("llc-changed", func(t *testing.T) {
		...
		info := &reconcileInfo{
			...
			portConfigs: map[string]*provisioner.PortConfig{
				"00:11:22:33:44:55": {
					SwitchPortConfig:    provisioner.SwitchPortConfig{Mode: "access", NativeVLAN: 100},
					LocalLinkConnection: &provisioner.LocalLinkConnection{SwitchID: "aa:bb:cc:dd:ee:ff", PortID: "Ethernet1/2"},
				},
			},
		}
...
func TestBuildAppliedPortConfigs(t *testing.T) {
	...
	info := &reconcileInfo{
		...
		portConfigs: map[string]*provisioner.PortConfig{
			"00:11:22:33:44:55": {
				SwitchPortConfig: provisioner.SwitchPortConfig{Mode: "access", NativeVLAN: 100, MTU: ptr.To(9000)},
			},
			"aa:bb:cc:dd:ee:ff": {
				SwitchPortConfig:    provisioner.SwitchPortConfig{Mode: "trunk", NativeVLAN: 1, AllowedVLANs: []int{10, 20}},
				LocalLinkConnection: &provisioner.LocalLinkConnection{SwitchID: "switch1", PortID: "Eth1/1"},
			},
		},
	}

	applied := buildAppliedPortConfigs(host, info)
...
func TestBuildNICNameToMACMap(t *testing.T) {
	t.Run("normal-nics", func(t *testing.T) {
		...
		result := buildNICNameToMACMap(host)
...
func TestBuildNICMacToNameMap(t *testing.T) {
	...
	t.Run("normal-nics", func(t *testing.T) {
		...
		result := buildNICMacToNameMap(host)
