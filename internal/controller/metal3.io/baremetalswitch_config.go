...
func generateSwitchConfig(ctx context.Context, c client.Client, namespace, credentialsPath string) (*switchConfigResult, error) {
...
func writeSwitchEntry(ctx context.Context, c client.Client, sw *metal3api.BareMetalSwitch, credentialsPath string, configEntries map[string][]byte, keyFiles map[string][]byte) error {
	...
	if !ok {
		return fmt.Errorf("credentials secret %s missing 'username' key", sw.Spec.Credentials.SecretName)
	}
	...
	if sw.Spec.Credentials.Type == metal3api.SwitchCredentialTypePublicKey {
		...
		if !ok {
			return fmt.Errorf("credentials secret %s missing 'ssh-privatekey' key for publickey auth", sw.Spec.Credentials.SecretName)
		}
		...
		if !ok {
			return fmt.Errorf("credentials secret %s missing 'password' key", sw.Spec.Credentials.SecretName)
		}
...
func updateSwitchConfigSecret(ctx context.Context, c client.Client, namespace, configSecretName, credentialSecretName, credentialPath string) error {
	// Generate the switch config from BareMetalSwitch CRDs
	result, err := generateSwitchConfig(ctx, c, namespace, credentialPath)
	if err != nil {
		return err
	}
