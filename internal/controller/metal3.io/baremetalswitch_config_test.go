...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := generateSwitchConfig(t.Context(), c, tt.namespace, tt.credentialPath)
...
`,
		...
		{
			name:      "error when switch credentials secret is missing",
			namespace: "test-ns",
			objects: []client.Object{
				&metal3api.BareMetalSwitch{
					ObjectMeta: metav1.ObjectMeta{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := updateSwitchConfigSecret(t.Context(), c, tt.namespace, testSwitchConfigsSecretName, testSwitchCredentialSecretName, testSwitchCredentialPath)
